...
constexpr std::remove_reference_t<_Tp> move(_Tp &&__t) { return static_cast<_Tp &&>(__t); }
...
namespace vm::utils {
    ...
    struct vector_t {
        ...
        explicit vector_t(const size_t capacity) : capacity(capacity),
                                                   data(static_cast<T *>(malloc(sizeof(T) * this->capacity))) {
        }

        vector_t() : vector_t(block_size) {
        }

        vector_t(const vector_t &other) : vector_t(other.capacity) {
            memcpy(data, other.data, capacity);
        }

        vector_t(vector_t &&other) noexcept : size(other.size), capacity(other.capacity), data(other.data) {
            other.data = nullptr;
        }
        ...
        void emplace(T &&t) {
            ...
            new(static_cast<void *>(data + size++)) T(move(t));
        }
        ...
        void emplace(addr_t ind, T &&t) {
            shrink_if_needed(size+1, block_size);
            if (size - ind > 0)
                memmove(data + ind + 1, data + ind, size++ - ind);
            new(static_cast<void *>(data + ind)) T(move(t));
        }

        void push(addr_t ind, const T &t) {
            shrink_if_needed(size+1, block_size);
            if (size - ind > 0)
                memmove(data + ind + 1, data + ind, size++ - ind);
    ...
    struct stream_t {
        ...
        explicit stream_t(FILE *stream) : stream(stream) {
        }
        ...
        T *read(T *data, const size_t n) {
            if (auto read = fread(data, sizeof(T), n, stream); read != sizeof(T) * n)
                memory_error("smth wrong with read stream");
            return data;
        }

        void write(T *data, const size_t n) {
            if (auto wrote = fwrite(data, sizeof(T), n, stream); wrote != sizeof(T) * n)
                memory_error("smth wrong with write stream");
        }
    };

#define char_slice(str) slice_t(str, sizeof(str)-1)
